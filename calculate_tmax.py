...
def main(met_dir, odir, land_sea_fname):
    ...
    nrows = 67
    ncols = 83
    tmax = np.ones((nrows,ncols)) * -999.9

    for year in range(st_yr, en_yr+1):
        print(year)
        fname = os.path.join(met_dir, "GSWP3.BC.Tair.3hrMap.%d.nc" % (year))
        tair, time_steps = open_file(fname)
        tmax_yr = tair.max(axis=0)

        tmax = np.where(tmax_yr > tmax, tmax_yr, tmax)

    tmax = np.where(sea_mask == 0, tmax, -999.9)

    ofile = open(os.path.join(odir, "tmax.bin"), "w")
    tmax.tofile(ofile)
